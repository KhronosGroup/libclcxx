...
kernel void worker(image1d<float4, image_access::sample> _d1f4r,
                   ...
                   sampler s, float lod)
{
    float4 _d1f4r_rf4f1 = _d1f4r.sample(s, float{0}, lod);
    float4 _d1af4r_rf4f1 = _d1af4r.sample(s, float{0}, lod);
    float4 _d2f4r_rf4f1 = _d2f4r.sample(s, float{0}, lod);
    float4 _d2af4r_rf4f1 = _d2af4r.sample(s, float{0}, lod);
    float4 _d2df1r_rf4f1 = _d2df1r.sample(s, float{0}, lod);
    float4 _d2adf1r_rf4f1 = _d2adf1r.sample(s, float{0}, lod);
    float4 _d3f4r_rf4f1 = _d3f4r.sample(s, float{0}, lod);

    // CHECK: call spir_func <4 x float> {{.*}}__spirv_ImageSampleExplicitLod
}